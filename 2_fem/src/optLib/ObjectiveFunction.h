...
public:
    ...
    void addFiniteDifferenceGradientTo(const VectorXd& x, VectorXd& grad) const {
        const double h = 1e-5;
        // TODO: Complete the finite difference computation for gradients
        // grad_FD = (f(x+h)-f(x-h))/2h
        // HINT: Use the function evaluate()
    ...
    void addFiniteDifferenceHessianEntriesTo(const VectorXd& x, std::vector<Triplet<double>>& hessianEntries) const {
        const double h = 1e-6;
        // TODO: Complete the finite difference computation for hessians
        // HINT: If your addFiniteDifferenceGradientTo() is correctly implemented, you can use addGradientTo()
        // NOTE: Only add a hessian entry if it is larger than 1e-12
        for (int i = 0; i < x.size(); ++i) {
            VectorXd dx(x.size());
            dx.setZero();
            dx[i] = h;
